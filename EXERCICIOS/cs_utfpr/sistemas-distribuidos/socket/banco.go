...
func init() {
	apps["banco"] = NewBancoApp(42069)
}

type BancoApp struct {
	sync.Mutex
	Port   int
	saldos map[int]float64
}

func (a *BancoApp) Deposito(conta int, valor float64) {
	if valor <= 0 {
		return
	}
	a.Lock()
...
func (a *BancoApp) Saldo(conta int) float64 {
	a.Lock()
	defer a.Unlock()
	val, ok := a.saldos[conta]
	if !ok {
...
func (a *BancoApp) Saque(conta int, valor float64) {
	if valor <= 0 {
		return
	}
	a.Lock()
...
func (a *BancoApp) Transferencia(conta int, outraConta int, valor float64) {
	if valor <= 0 {
		return
	}
	a.Lock()
...
func (a *BancoApp) Client(args []string) error {
...
func (a *BancoApp) Server(args []string) error {
