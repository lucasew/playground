...
% Posição dos textos informativos
\def\textoX{2.8}
\def\textoY{4.8}
\def\textoLargura{2.5cm}  % Largura fixa da caixa de texto
% ==============================================
...
\begin{document}
	...
	\foreach \n in {2,3,...,\maxIters}{
		...
		\begin{tikzpicture}[
			...
			% Textos em caixa de largura fixa
			\node[anchor=north east, inner sep=2pt, text width=\textoLargura, align=left] at (\textoX,\textoY) {
				\funcaoNome \\
				$A_{\n} = \fpeval{round(\soma,5)}$ \\
				$E_{\n} = \fpeval{round(\erro,5)}\%$
			};
